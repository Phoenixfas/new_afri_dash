...
export default function ViewRegModal({path}: any) {
    ...
    const regData = useAppSelector(state => state.activeStartup)
            ...
            <motion.div variants={variants} initial={"closed"} animate={"open"} exit={"closed"} className={`fixed z-10 top-0 blog-modal left-0 w-full h-screen bg-[#00000077] flex py-16 justify-center overflow-y-auto`}>
              <motion.div initial={{y: "100%"}} animate={{y: 0}}  transition={{duration: .3}} className='relative max-w-[800px] h-fit flex flex-col bg-white rounded-2xl p-20'>
                ...
                <div className=' w-full'>
                  ...
                  <div className='flex  flex-wrap gap-1 mb-4 flex-col'>
                    <div className='text-xl font-bold text-black'>Company Description</div>
                    <div className='text-xl font-light text-black '>{regData.company_description}</div>
                  </div>
                  <div className='flex  flex-wrap gap-1 mb-4 flex-col'>
                    <div className='text-xl font-bold text-black'>Company Stage</div>
                    <div className='text-xl font-light text-black '>{regData.company_stage}</div>
                  </div>
                  <div className='flex  flex-wrap gap-1 mb-4 flex-col'>
                    <div className='text-xl font-bold text-black'>Company Industry</div>
                    <div className='text-xl font-light text-black '>{regData.company_industry}</div>
                  </div>
                  <div className='flex  flex-wrap gap-1 mb-4 flex-col'>
                    <div className='text-xl font-bold text-black'>Company Size</div>
                    <div className='text-xl font-light text-black '>{regData.company_size}</div>
                  </div>
