...
export default function ViewRegModal({path}: any) {
    ...
    const regData = useAppSelector(state => state.activeReg)
            ...
            <motion.div variants={variants} initial={"closed"} animate={"open"} exit={"closed"} className={`fixed z-10 top-0 blog-modal left-0 w-full h-screen bg-[#00000077] flex py-16 justify-center overflow-y-auto`}>
              <motion.div initial={{y: "100%"}} animate={{y: 0}}  transition={{duration: .3}} className='relative max-w-[800px] h-fit flex flex-col bg-white rounded-2xl p-20'>
                ...
                <div className=' w-full'>
                  ...
                  <div className='flex  flex-wrap gap-1 mb-4 flex-col'>
                    <div className='text-xl font-bold text-black'>Telegram</div>
                    <div className='text-xl font-light text-black '>{regData.telegram}</div>
                  </div>
                  <div className='flex  flex-wrap gap-1 mb-4 flex-col'>
                    <div className='text-xl font-bold text-black'>Gender</div>
                    <div className='text-xl font-light text-black '>{regData.gender}</div>
                  </div>
                  {regData.have_team && <div className='flex  flex-wrap gap-1 mb-4 flex-col'>
                    <div className='text-xl font-bold text-black'>Have a Team</div>
                    <div className='text-xl font-light text-black '>{regData.have_team ? <FaRegCheckCircle color='green' /> : <CgCloseO color='red' />}</div>
                  </div>}
                  {regData.previous_experience && <div className='flex  flex-wrap gap-1 mb-4 flex-col'>
                    <div className='text-xl font-bold text-black'>Have Previous Experience</div>
                    <div className='text-xl font-light text-black '>{regData.previous_experience ? <FaRegCheckCircle color='green' /> : <CgCloseO color='red' />}</div>
                  </div>}
                  {regData.why_participate && <div className='flex  flex-wrap gap-1 mb-4 flex-col'>
                    <div className='text-xl font-bold text-black'>Why Participate</div>
                    <div className='text-xl font-light text-black '>{regData.why_participate}</div>
                  </div>}
                  {regData.additional_info && <div className='flex  flex-wrap gap-1 mb-4 flex-col'>
                    <div className='text-xl font-bold text-black'>Additional Information</div>
                    <div className='text-xl font-light text-black '>{regData.additional_info}</div>
                  </div>}
                ...
                }} ><FaTrash /> DELETE REGISTERER</button> */}
              </motion.div>
            </motion.div>
        )}
      </AnimatePresence>
