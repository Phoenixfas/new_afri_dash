...
export default function SubscribersList({d}: SubscribersListProps) {
  ...
  return (
    <div className='min-h-[60vh] gap-5 w-full flex flex-col flex-wrap items-start'>
        {sortByNewest && subs.sort((b1:any, b2:any) => b2.createdAt - b1.createdAt).map((sub: any) => (
            <div key={sub.id} className='w-full p-3 gap-5 shadow-lg bg-white hover:scale-[1.02] duration-300 rounded-xl sub-card flex flex-wrap items-center justify-between'>
                ...
                <button className="p-3 bg-red-500 text-lg w-fit  cursor-pointer text-white rounded-md" onClick={() => {
                    deleteSubscriber({variables: {id: sub.id}, refetchQueries: [{query: GET_SUBSCRIBERS}]})
                }} ><FaTrash /></button>
            </div>
        ))}
        {!sortByNewest && subs.sort((b1:any, b2:any) => b1.createdAt - b2.createdAt).map((sub: any) => (
            <div key={sub.id} className='w-full p-3 gap-5 shadow-lg bg-white hover:scale-[1.02] duration-300 rounded-xl sub-card flex flex-wrap items-center justify-between'>
                <div className='text-sm text-black font-bold'>{sub.email}</div>
